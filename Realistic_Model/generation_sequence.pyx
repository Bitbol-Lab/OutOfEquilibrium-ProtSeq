...
cdef class Creation_MSA_Generation:
    ...
    def msa_no_phylo_setT(self, int n_sequences, int n_flip_equi, double temperature):
        cdef:
            int index_msa
            char[:,::1] msa = np.random.randint(0,high=self.Number_state_spin, 
                                size = (n_sequences,self.Number_of_Node)
                                ,dtype = np.int8)
        for index_msa in prange(msa.shape[0],nogil=True,schedule='dynamic'):
            self.mcmc_temp(n_flip_equi, msa[index_msa],temperature)
        return np.asarray(msa)
    
    def msa_no_phylo_startmsa(self, int n_sequences, int n_flip_equi,char[:,::1] msa, double temperature):
        cdef:
            int index_msa
            # char[:,::1] msa = np.random.randint(0,high=self.Number_state_spin, 
            #                     size = (n_sequences,self.Number_of_Node)
    ...
    cdef inline void mcmc_temp(self, int Number_of_Mutation, char[::1] L_Spin, double temperature) nogil:  
        cdef:
            int selected_node, new_state, c_mutation = 0
            double Prob, de
        while c_mutation<Number_of_Mutation:   
